...
export class Replicache {
   ...
   */
  pendingMutations: (Mutation & { status: 'waiting' | 'pending' | 'pushed', kvUpdates: Map<string, any> })[] = [];
  ...
  #subscriptions = new Map<
    (tx: ReturnType<typeof Replicache.prototype.createReadTransaction>) => Promise<any>,
    { keysReadOnLastExecution: Set<string>, onQueryCbChanged: (res: any) => void }
  >();
  ...
  async query(cb: (tx: ReturnType<typeof Replicache.prototype.createReadTransaction>) => Promise<any>) {
  ...
  async #runAndUpdateSubscription(cb: (tx: ReturnType<typeof Replicache.prototype.createReadTransaction>) => Promise<any>, changedKeys: Set<string>) {
    const tx = this.createReadTransaction();
  ...
  #getKeys(): Set<string> {
    const keySet = new Set([...this.kv.keys(), ...this.pendingMutations.flatMap(mutation => Array.from(mutation.kvUpdates.keys()))]);
    return keySet;
  }
