...
export class ReplicacheCore {
  ...
  getSerializedFunctionString(mutatorName: string) {
    ...
    // // Check if it's a method shorthand or class method
    // if (/^[a-zA-Z_$][a-zA-Z0-9_$]*\s*\(/.test(str)) {
    //   const funcStr = `{ invoke: (tx, args) => {
    //     return {
    //       ${str}
    //     }[${mutatorName}](tx, args)
    //   }`
